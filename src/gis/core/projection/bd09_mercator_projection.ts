 ...
 * bd09 墨卡托投影
 */
export class BD09MercatorProject extends BaseProjection {

    private isWgs84 = false;

    public constructor (isWgs84: boolean = false, ellipsoid?: Ellipsoid) {
        super(ellipsoid);
    ...
    public project (cartographic: Cartographic, out?: Cartesian3): Cartesian3 {
        ...
        let result = CoordinateTransform.wgs84togcj02(
            math.toDegree(cartographic.longitude),
            math.toDegree(cartographic.latitude)
        )
        result = CoordinateTransform.gcj02tobd09(result[0], result[1])
        result[0] = Math.min(result[0], 180)
        result[0] = Math.max(result[0], -180)
        result[1] = Math.min(result[1], 74.000022)
        result[1] = Math.max(result[1], -71.988531)
        const point = this.lngLatToPoint({
            lng: result[0],
            lat: result[1]
        });
        out = out || new Cartesian3();
    ...
    convertMC2LL (point) {
        ...
        let lnglat: Record<string, number> = {}
        if (this.isWgs84) {
    ...
    convertLL2MC (point) {
        ...
        if (this.isWgs84) {
            let mercator: Record<string, number> = {}
